...
function accentFoldedHighlight(str, fragment, wrapTag) {
    wrapTag = wrapTag || "b";
    var str_folded = accentFold(str).toLowerCase().replace(/[<>]+/g, "");
    var q_folded = accentFold(fragment).toLowerCase().replace(/[<>]+/g, "");

    // create an intermediary string with hilite hints
    // example: fulani<lo> <lo>pez
    var re = new RegExp(q_folded, "g");
    var hiliteHints = str_folded.replace(re, "<"+q_folded+">");
    ...
    for (var i = 0; i < hiliteHints.length; i++) {
        var c = str.charAt(spos);
        var h = hiliteHints.charAt(i);
        if (h === "<") {
            highlighted += "<"+ wrapTag +">";
        } else if (h === ">") {
            highlighted += "</"+ wrapTag +">";
