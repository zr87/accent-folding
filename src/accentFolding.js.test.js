...
describe("AccentFolding", () => {
	...
	describe("constructor", () => {
		it("should throw TypeError if customMap is not an array", () => {
			...
			expect(() => new AccentFolding("not an array")).toThrow(
				"customMap must be an array",
			);
		});
	});
	...
	describe("highlightMatch", () => {
		...
		it("should throw TypeError if wrapTag is not a string", () => {
			expect(() =>
				accentFolder.highlightMatch("test", "es", 123),
			).toThrow(TypeError);
			expect(() =>
				accentFolder.highlightMatch("test", "es", 123),
			).toThrow("wrapTag must be a string");
		});
		...
		it("handles special characters in fragment", () => {
			expect(accentFolder.highlightMatch("a+b=c", "+")).toBe(
				"a<b>+</b>b=c",
			);
		});
	...
	describe("replace", () => {
		...
		it("should throw TypeError if input is not a string", () => {
			...
			expect(() => accentFolder.replace(123)).toThrow(
